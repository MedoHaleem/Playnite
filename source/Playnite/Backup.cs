    ...
    public class Backup
    {
        ...
        public static void BackupData(BackupOptions options, CancellationToken cancelToken)
        {
            ...
            using (var archive = new ZipArchive(zipFile, ZipArchiveMode.Create))
            {
                ...
                {
                    // Library
                    foreach (var libFile in Directory.EnumerateFiles(Path.Combine(options.LibraryDir)))
                    {
                ...
                if (options.BackupItems.Contains(BackupDataItem.Themes) && Directory.Exists(themesDir))
                {
                    void packThemes(ApplicationMode mode)
                    {
                        ...
                        {
                            foreach (var themeDir in Directory.EnumerateDirectories(modeThemesDir))
                            {
        ...
        archiveDone:
            ...
            else
            {
                ...
                var files = Directory.EnumerateFiles(backupDir, $"{autoBackupFileName}*.zip").Where(a => Regex.IsMatch(a, autoBackupFilePattern)).OrderBy(a => a).ToArray();
        ...
        public static void RestoreBackup(BackupRestoreOptions options)
        {
            ...
            using (var archive = new ZipArchive(zipFile, ZipArchiveMode.Read))
            {
                ...
                {
                    foreach (var file in Directory.EnumerateFiles(options.LibraryDir))
                    {
                        File.Delete(file);
                    }
                ...
                void unpackThemeBackupDir(bool restore, string outputDir, string dirPrefix)
                {
                    ...
                    if (restore && archive.Entries.Any(a => a.FullName.StartsWith(dirPrefix, StringComparison.OrdinalIgnoreCase)))
                    {
                        void cleanThemeModeDir(ApplicationMode mode)
                        {
                            ...
                            foreach (var dir in Directory.EnumerateDirectories(modeDir))
                            {
                            ...
                            foreach (var file in Directory.EnumerateFiles(modeDir))
                            {
