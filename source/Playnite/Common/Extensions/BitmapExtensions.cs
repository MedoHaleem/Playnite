...
namespace System.Drawing.Imaging
{
    ...
    public static partial class BitmapExtensions
    {
        ...
        public static Task<BitmapSource> BitmapFromFileAsync(string filePath, BitmapLoadProperties loadProperties = null, CancellationToken cancelToken = default)
        {
            return RunDecodeAsync(() => BitmapFromFile(filePath, loadProperties), cancelToken);
        }

        public static Task<BitmapSource> BitmapFromStreamAsync(Stream stream, BitmapLoadProperties loadProperties = null, CancellationToken cancelToken = default)
        {
            return RunDecodeAsync(() => BitmapFromStream(stream, loadProperties), cancelToken);
        }

        private static async Task<BitmapSource> RunDecodeAsync(Func<BitmapSource> decodeAction, CancellationToken cancelToken)
        {
            await decodeSemaphore.WaitAsync(cancelToken).ConfigureAwait(false);
            try
            {
                return await Task.Run(decodeAction, cancelToken).ConfigureAwait(false);
            }
            finally
            {
