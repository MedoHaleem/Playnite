...
namespace Playnite
{
    public static class Diagnostic
    {
        ...
        private static List<string> GetPlayniteFilesList()
        {
            ...
            foreach (var file in Directory.EnumerateFiles(progPath, "*.*", SearchOption.TopDirectoryOnly))
            {
                try
                {
        ...
        public static void CreateDiagPackage(string path, string userActionsDescription, DiagnosticPackageInfo packageInfo)
        {
            ...
            using (FileStream zipToOpen = new FileStream(path, FileMode.Open))
            {
                using (ZipArchive archive = new ZipArchive(zipToOpen, ZipArchiveMode.Update))
                {
                    ...
                    {
                        foreach (var cfg in Directory.EnumerateFiles(PlaynitePaths.ConfigRootPath, "*.json"))
                        {
                    ...
                    {
                        foreach (var cfg in Directory.EnumerateFiles(PlaynitePaths.ExtensionsDataPath, "config.json", SearchOption.AllDirectories))
                        {
                            var fileInfo = new FileInfo(cfg);
                    ...
                    // Add log files
                    foreach (var logFile in Directory.EnumerateFiles(PlaynitePaths.ConfigRootPath, "*.log", SearchOption.TopDirectoryOnly))
                    {
