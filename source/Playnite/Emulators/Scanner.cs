    ...
    public class EmulatorScanner
    {
        private static readonly ILogger logger = LogManager.GetLogger();
	        ...
	        public static List<ScannedEmulator> SearchForEmulators(string path, IList<EmulatorDefinition> definitions, CancellationToken cancelToken)
	        {
	            ...
	            var detectionRegexCache = new Dictionary<EmulatorDefinitionProfile, Regex>();

	            Regex getDetectionRegex(EmulatorDefinitionProfile profile)
	            {
	                if (detectionRegexCache.TryGetValue(profile, out var cached))
	                {
	                    return cached;
	                }

	                var detectionStr = profile.InstallationFile;
	                if (detectionStr.IsNullOrEmpty())
	                {
	                    detectionStr = profile.StartupExecutable;
	                }

	                if (detectionStr.IsNullOrEmpty() && !PlayniteEnvironment.ThrowAllErrors)
	                {
	                    detectionRegexCache[profile] = null;
	                    return null;
	                }

	                try
	                {
	                    cached = new Regex(detectionStr ?? string.Empty, RegexOptions.IgnoreCase);
	                }
	                catch (Exception e) when (!PlayniteEnvironment.ThrowAllErrors)
	                {
	                    logger.Error(e, $"Failed to create emulator detection regex from \"{detectionStr}\".");
	                    cached = null;
	                }

	                detectionRegexCache[profile] = cached;
	                return cached;
	            }
                ...
                foreach (var definition in definitions)
                {
                    ...
                    var importId = definition.Id + currentDir;
	                    ...
	                    {
	                        var reqMet = true;
	                        var regex = getDetectionRegex(defProfile);
	                        if (regex == null)
	                        {
	                            continue;
	                        }
    ...
    public class GameScanner
    {
            ...
            Action<string> fileScanCallback = null)
        {
            ...
            bool scanSubfolders,
	            ...
	            Action<string> fileScanCallback = null)
	        {
	            Dictionary<string, ScannedGame> mergedGames = null;
	            void addRom(ScannedRom rom)
	            {
	                if (mergeRelatedFiles)
	                {
	                    if (mergedGames == null)
	                    {
	                        mergedGames = new Dictionary<string, ScannedGame>(StringComparer.Ordinal);
	                    }

	                    if (mergedGames.TryGetValue(rom.Name.SanitizedName, out var existing))
	                    {
	                    ...
	                    else
	                    {
	                        ...
	                        mergedGames[rom.Name.SanitizedName] = game;
            ...
            List<string> dirs;
	            try
	            {
	                ...
	                dirs = Directory.EnumerateDirectories(directory).ToList();
	            }
	            catch (Exception e) when (!PlayniteEnvironment.ThrowAllErrors)
	            {
