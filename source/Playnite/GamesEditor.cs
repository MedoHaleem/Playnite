...
namespace Playnite
{
    ...
    public class GamesEditor : ObservableObject, IDisposable
    {
        ...
        {
            UpdateGameSize(game, onlyIfDataMissing, updateGameOnLibrary, checkLastScanDate, CancellationToken.None);
        }

        public void UpdateGameSize(Game game, bool onlyIfDataMissing, bool updateGameOnLibrary, bool checkLastScanDate, CancellationToken cancelToken)
        {
            if (!game.IsInstalled)
            {
                return;
            ...
            try
            {
                scanSize = CalculateGameSize(game, checkLastScanDate, cancelToken);
            }
        ...
        {
            return CalculateGameSize(game, checkLastScanDate, CancellationToken.None);
        }

        public ulong? CalculateGameSize(Game game, bool checkLastScanDate, CancellationToken cancelToken)
        {
            long size = 0;
            ...
            else
            {
                ...
                size = FileSystem.GetDirectorySize(expandedGame.InstallDirectory, AppSettings.InstallSizeScanUseSizeOnDisk, cancelToken, true);
            }
        ...
        public void UpdateGameSizeWithDialog(Game game, bool onlyIfDataMissing, bool updateGameOnLibrary)
        {
            ...
            Dialogs.ActivateGlobalProgress((a) =>
            {
                try
                {
                    UpdateGameSize(game, onlyIfDataMissing, updateGameOnLibrary, false, a.CancelToken);
                }
                catch (Exception e)
                {
                    Dialogs.ShowMessage(
            ...
            Dialogs.ActivateGlobalProgress((a) =>
            {
                ...
                using (Database.BufferedUpdate())
                {
                    foreach (var game in games)
                    {
                        ...
                        try
                        {
                            UpdateGameSize(game, onlyIfDataMissing, true, false, a.CancelToken);
                        }
                        catch (OperationCanceledException)
                        {
                            break;
